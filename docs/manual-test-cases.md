...
This same model was used to select which cases were promoted to automated coverage: the two highest-severity defects (checkout-blocking, add-to-cart failures) received full automated, evidence-generating coverage, extended to a third area (product listing/image integrity) once a clean opportunity for data-driven, parameterized coverage was identified. Sort (TC-2.7) was deliberately kept manual-only despite being a confirmed defect, as it is a usability-degrading rather than transaction-blocking issue, and the fixed automation budget was allocated to higher-risk findings first.
...
### TC-2.1 — Product listing displays complete, correct item details **[Automated]**
...
- **Pass/Fail:** Pass if all fields present and images are unique per product; Fail if any product is missing details or shares an image with another.
...
### TC-2.6 — Problem user: product images are duplicated/incorrect **[Automated]**

- **Priority:** Medium — cosmetic but catalog-integrity-relevant; caught as part of TC-2.1's automated equivalent for this user.
...
- **Pass/Fail:** Fail (documented defect — see BUG-004).
...
### TC-3.2 — Standard user can add multiple items to the cart **[Automated]**
...
- **Pass/Fail:** Pass if count and contents are correct; Fail otherwise.
...
### TC-3.5 — Problem user: "Add to Cart" fails for specific items **[Automated]**
...
- **Pass/Fail:** Fail (documented defect — see BUG-002).

### TC-3.6 — Problem user: "Add to Cart" functions correctly for unaffected items **[Automated]**
...
- **Pass/Fail:** Pass if all 3 unaffected items add correctly, confirming the defect is isolated to the specific items identified in TC-3.5.
...
- **Pass/Fail:** Pass if confirmation is reached; Fail if any step is blocked or errors.
...
- **Pass/Fail:** Fail (documented defect — see BUG-001; highest-severity finding).
